  ...
  return <>
      <Dialog open={isOpen} onOpenChange={handleModalOpenChange}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-auto bg-white/95 backdrop-blur-sm border-0 shadow-2xl">
          ...
          {/* Navigation Arrows */}
          {hasPrevious && onPrevious && <Button variant="ghost" size="sm" onClick={onPrevious} className="absolute left-4 top-1/2 -translate-y-1/2 z-10 h-10 w-10 p-0 bg-white/80 hover:bg-white/90 shadow-md rounded-full">
              <ChevronLeft className="h-5 w-5" />
            </Button>}
          
          {hasNext && onNext && <Button variant="ghost" size="sm" onClick={onNext} className="absolute right-4 top-1/2 -translate-y-1/2 z-10 h-10 w-10 p-0 bg-white/80 hover:bg-white/90 shadow-md rounded-full">
            ...
            {isEditing ? <RecapCardEditor log={log} onSave={handleSave} onCancel={() => setIsEditing(false)} /> : <>
                ...
                {log.notes && <div className="bg-gray-50/80 rounded-lg p-4 border border-gray-200/50 py-[16px]">
                    ...
                    {log.lesson && log.lesson.toLowerCase() !== 'general' ? (
                      ...
                      <div className="mb-4">
                        <div className="inline-block bg-gradient-to-r from-gray-50 to-gray-100 border border-gray-200 rounded-lg px-3 py-2">
                          <h3 className="text-base font-semibold text-gray-700 underline decoration-gray-400 decoration-2 underline-offset-2">
                            Notes
                          </h3>
                        </div>
                      </div>
                ...
                {/* Achievements */}
                {log.achievements && <div className="bg-green-50/80 rounded-lg p-4 border border-green-200/50">
                ...
                {/* Attachments */}
                {log.images && log.images.length > 0 && <div className="bg-blue-50/80 rounded-lg p-4 border border-blue-200/50">
                    <h4 className="text-sm font-medium text-blue-700 mb-3 flex items-center">
                      <Target className="h-4 w-4 mr-2" />
                      Attachments ({log.images.length})
                    </h4>
                    <div className="grid grid-cols-6 gap-3">
                      {log.images.map((src, index) => <div key={index} className="relative aspect-square rounded-lg overflow-hidden border border-blue-200 cursor-pointer hover:shadow-md transition-all duration-200 group" onClick={() => handleImageClick(index)}>
                          <img src={src} alt={`attachment-${index}`} className="w-full h-full object-cover transition-transform duration-200 group-hover:scale-105" onError={e => {
      ...
      {log.images && log.images.length > 0 && <ImageLightbox images={log.images} initialIndex={lightboxIndex} isOpen={lightboxOpen} onClose={() => setLightboxOpen(false)} />}
    </>;
};
